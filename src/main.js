...
function calcCart() {
  totalAmt = 0; // 총 할인 후 금액
  itemCnt = 0; // 총 수량
  var cartItems = cartDisp.children; // 장바구니 아이템들
  var subTot = 0; // 총 할인 전 금액
  for (var i = 0; i < cartItems.length; i++) {
    (function () {
      ...
      totalAmt += itemTot * (1 - disc); // 할인 적용 금액
    })();
  }
  let discRate = 0; // 총 할인률
  if (itemCnt >= 30) {
    var bulkDisc = totalAmt * 0.25; // 대량 할인가
    var itemDisc = subTot - totalAmt; // 개별 할인가
    if (bulkDisc > itemDisc) {
      totalAmt = subTot * (1 - 0.25); // 최종 =  대량 할인 적용
      discRate = 0.25; // 최종 =  대량 할인율
    } else {
      discRate = (subTot - totalAmt) / subTot; // 최종 = 개별 할인률 계산
    }
  } else {
    discRate = (subTot - totalAmt) / subTot; // 최종 = 개별 할인률 계산
  }
  ...
  if (new Date().getDay() === 2) {
    totalAmt *= 1 - 0.1; // 최종 = 화요일 할인 적용
    discRate = Math.max(discRate, 0.1); // 최종 = 화요일 할인율
  }
