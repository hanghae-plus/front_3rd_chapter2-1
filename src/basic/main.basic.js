...
let itemCnt=0;
// El생성
const createElementWithHandler = (tagName, { 
  id, 
  className, 
  textContent, 
  innerHTML,
  ...
  if (onClick) element.addEventListener('click', onClick); // 클릭 이벤트 등록
  
  parent.appendChild(element); // 부모에 추가
  return element;
};
...
// cont
const cont = createElementWithHandler('div', {
  className: 'bg-gray-100 p-8',
}, root);
//wrap
const wrap = createElementWithHandler('div', {
  className: 'max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl p-8',
}, cont);

const title = createElementWithHandler('h1', {
  className: 'text-2xl font-bold mb-4',
  textContent: '장바구니',
}, wrap);

const cart = createElementWithHandler('div', {
  id: 'cart-items',
  onClick:($event)=> {
    ...
    if(tgt.classList.contains('quantity-change') || tgt.classList.contains('remove-item')) {
      ...
      let prod=prodList.find(function (p) { return p.id === prodId; });
...
const cartTotal = createElementWithHandler('div', {
  id: 'cart-total',
  className: 'text-xl font-bold my-4',
}, wrap);

const productSelect = createElementWithHandler('select', {
  id: 'product-select',
  className: 'border rounded p-2 mr-2',
}, wrap);

const addToCartBtn = createElementWithHandler('button', {
  id: 'add-to-cart',
  className: 'bg-blue-500 text-white px-4 py-2 rounded',
  textContent: '추가',
  onClick: ()=> {
    let selItem=productSelect.value;
    let itemToAdd=prodList.find(function (p) { return p.id === selItem; });
    if(itemToAdd && itemToAdd.q > 0) {
      ...
      if(item) {
        ...
        createElementWithHandler('div', {
          id:itemToAdd.id,
          className: 'flex justify-between items-center mb-2',
          innerHTML:'<span>' + itemToAdd.name + ' - ' + itemToAdd.val + '원 x 1</span><div>' +
...
const stockStatus = createElementWithHandler('div', {
  id: 'stock-status',
...
function main() {
  ...
  setTimeout(function () {
    setInterval(function () {
      if(lastSel) {
        let suggest=prodList.find(function (item) { return item.id !== lastSel && item.q > 0; });
        if(suggest) {
