...
elements.cartDisp.addEventListener('click', (event) => {
  ...
  let quantity_change = target.classList.contains('quantity-change');
  let remove_item = target.classList.contains('remove-item');

  if (quantity_change || remove_item) {
    let prodId = target.dataset.productId;
    let itemElem = document.getElementById(prodId);
    let span_textContent = parseInt(itemElem.querySelector('span').textContent.split('x ')[1]);
